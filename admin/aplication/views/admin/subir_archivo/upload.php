...
// set variables
if($_SESSION['ruta_upload'] == "" ) $dir_dest = (isset($_GET['dir']) ? $_GET['dir'] : 'test');
else $dir_dest = $_SESSION['ruta_upload'];
...
if ((isset($_POST['action']) ? $_POST['action'] : (isset($_GET['action']) ? $_GET['action'] : '')) == 'simple') {
    ...
    if ($handle->uploaded) {
        ...
        if ($handle->processed) {
            // everything was fine !
            echo '<p class="result">';
            echo '  <b>File uploaded with success1</b><br />';
            echo '  File: <a href="subir_archivo/'.$dir_pics.'/' . $handle->file_dst_name . '">' . $handle->file_dst_name . '</a>';
    ...
    if ($handle->uploaded) {
        ...
        if ($handle->processed) {
            ...
            echo '  <img src="subir_archivo/'.$dir_pics.'/' . $handle->file_dst_name . '" />';
    ...
    if ($handle->uploaded) {
        ...
        if ($handle->processed) {
            ...
            echo ' <img src="subir_archivo/sube_ok.jpg" width="30px">'; // .
			echo ' <input type="hidden" name="ruta" id="ruta" value="'.$handle->file_dst_name.'">';
            echo '   (' . round(filesize($handle->file_dst_pathname)/256)/4 . 'KB)';
            echo '</p>';
        } else {
            // one error occured
