...
//			$this->items_per_page = $this->default_ipp;
		...
		if($this->num_pages > 10)
		{
			...
			for($i=1;$i<=$this->num_pages;$i++)
			{
				...
				{
					$this->return .= ($i == $this->current_page And $_GET['page'] != 'All') ? "<a title=\"ir a pag. $i of $this->num_pages\" class=\"current\" href=\"#\">$i</a> ":"<a class=\"paginate\" title=\"ir a pag $i of $this->num_pages\" href=\"$_SERVER[PHP_SELF]?page=$i&ipp=$this->items_per_page$this->querystring\">$i</a> ";
				}
	...
	function display_items_per_page()
	{
		...
		return "<span class=\"paginate\">Items por pag.:</span><select class=\"paginate\" onchange=\"window.location='$_SERVER[PHP_SELF]?page=1&ipp='+this[this.selectedIndex].value+'$this->querystring';return false\">$items</select>\n";
	}
	function display_jump_menu()
	{
		...
		return "<span class=\"paginate\">Pag:</span><select class=\"paginate\" onchange=\"window.location='$_SERVER[PHP_SELF]?page='+this[this.selectedIndex].value+'&ipp=$this->items_per_page$this->querystring';return false\">$option</select>\n";
	}
